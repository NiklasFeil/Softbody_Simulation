    ...
    for (int k = 0; k < m_grid_dim; ++k) {
        for (int j = 0; j < m_grid_dim; ++j) {
            for (int i = 0; i < m_grid_dim; ++i) {
                double x = -size/2.0 + i * 2;
                double y = -size/2.0 + j * 2;
                double z = -size/2.0 + k * 2;
    ...
    for(int k = 0; k < m_solver_iterations; k++) {
        ...
        if (Cj != 0.0) {
            ...
            for(auto& triangle: m_triangles) {
                ...
                // TODO: This might not assign correctly
                //dCj.segment(3 * first, 3) += second_pos.cross(third_pos) / 6.0;
                //dCj.segment(3 * second, 3) += third_pos.cross(first_pos) / 6.0;
                //dCj.segment(3 * third, 3) += first_pos.cross(second_pos) / 6.0;
